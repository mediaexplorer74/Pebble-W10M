...
namespace P3bble.Core
{
    ...
    public class P3bble : IP3bble, IDisposable
    {
        ...
        // Used to asynchronise calls to the Pebble to make more natural for the API consumer...
        ...
        public async Task<bool> TransferOwnership(string socketId)
        {
            //await _protocol.Socket.CancelIOAsync();
            //Dirtyfix
            bool isError=true;
            do
            {
                ...
                {
                    //await _protocol.Socket.CancelIOAsync();
                    //_protocol.Socket.InputStream.Dispose();
                    //_protocol.Socket.OutputStream.Dispose();
                    this._protocol.MessageReceived -= this.ProtocolMessageReceived;
                    _protocol.socketID = socketId;
                    //Protocol.ctsSR.Cancel(false);
                    _protocol._isRunning = false;
                ...
                {
                    isError = true;
                    PCL.ServiceLocator.Logger.WriteLine(DateTime.Now.ToString() + " P3bble: " + ex.Message);
                }
            } while (isError);
            return true;
        }
        ...
        public string DisplayName
        {
            get
            {
                if (PeerInformation != null)
                    ...
                    return _deviceService.Device.Name;
            }
        }
        ...
        public async Task<bool> ConnectAsync(
            Guid backgroundtask,Windows.Networking.Sockets.StreamSocket socket = null)
        {
            ...
            {
                if (PeerInformation != null)
                    this._protocol = await Protocol.CreateProtocolAsync(PeerInformation,socket,backgroundtask);
                else
        ...
        public /*async*/ void Disconnect()
        {
            if (this._protocol != null)
            {
                ...
                {
                    PCL.ServiceLocator.Logger.WriteLine(ex.StackTrace);
                    throw ex;
                }
            }
        ...
        public async Task<InstalledApplication> GetRunningAppsAsync()
        {
            ServiceLocator.Logger.WriteLine("GetRunningAppsAsync");
            var result = await this.SendMessageAndAwaitResponseAsync<AppManagerMessage>(new AppManagerMessage(AppManagerAction.ListRunning));
            if (result != null)
            {
                return result.RunningApplication;
            }
            else
            {
                throw new TimeoutException();
        ...
        public async Task InstallAppAsync(Bundle bundle)
        {
            ...
            await this.LaunchAppAsync((await GetInstalledAppsAsync()).ApplicationsInstalled.Where(x => x.Id == BitConverter.ToUInt32(bundle.Application.Uuid.ToByteArray(),0)).FirstOrDefault());
        }
        ...
        /// An async task to wait
        /// </returns>
        public async Task<bool> LaunchAppAsync(InstalledApplication app)
        {
            var msg = new AppMessage(Endpoint.Launcher)
            {
                AppUuid = BitConverter.GetBytes(app.Id),
                AppIndex = app.Index,
        ...
        private static async Task<List<P3bble>> FindPebbles()
        {
            ...
            try
            {
                ...
                //Specific code for desktop-WUP                
                bool isWindowsPhonePresent = ApiInformation.IsApiContractPresent("Windows.Phone.PhoneContract", 1);
                if (!isWindowsPhonePresent)
                {
                    RfcommServiceId serviceId = RfcommServiceId.FromUuid(Guid.Parse("00000000-deca-fade-deca-deafdecacaff"));
                    var devices = await DeviceInformation.FindAllAsync(RfcommDeviceService.GetDeviceSelector(serviceId));
            ...
            switch (message.Endpoint)
            {
                ...
                case Endpoint.MusicControl:
                    ...
                    if (this.MusicControlReceived != null && musicMessage != null && musicMessage.ControlAction != MusicControlAction.Unknown
                        && musicMessage.ControlAction != MusicControlAction.Unkown2)
                    {
        ...
        private Task<T> SendMessageAndAwaitResponseAsync<T>(P3bbleMessage message, int millisecondsTimeout = 20000)
