    ...
    public class Bundle
    {
        ...
        internal async Task Initialise()
        {
            ...
            if (manifestEntry == null)
                manifestEntry = this._bundle.Entries.Where(e => string.Compare(e.Key, "aplite/manifest.json", StringComparison.OrdinalIgnoreCase) == 0).FirstOrDefault();

            foreach (var item in this._bundle.Entries)
            {
                PCL.ServiceLocator.Logger.WriteLine(item.Key);
            }
            if (manifestEntry == null)
            {
        ...
        private async Task<byte[]> ReadFileToArray(string file, int size)
        {
            ...
            if (entry == null)
                entry = this._bundle.Entries.Where(e => string.Compare(e.Key, "aplite/" + file, StringComparison.OrdinalIgnoreCase) == 0).FirstOrDefault();
